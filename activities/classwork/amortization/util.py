...
class Amortization(object):
    ...
    def get_plot(self):
        ...
        plt.ylabel('Amount $')
        plt.subplot(2, 2, 2)
        plt.plot(list(np.arange(self.n) + 1), list(tabla.iloc[1:, 1]))
        plt.title('Interest')
        plt.xlabel('Periods')
        plt.ylabel('Interest $')
        plt.subplot(2, 2, 3)
        plt.plot(list(np.arange(self.n) + 1), list(tabla.iloc[1:, 2]))
        plt.title('Annuity')
        plt.xlabel('Periods')
        plt.ylabel('Annuity $')
        plt.subplot(2, 2, 4)
        plt.plot(list(np.arange(self.n) + 1), list(tabla.iloc[1:, 3]))
